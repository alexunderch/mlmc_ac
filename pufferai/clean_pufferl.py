...
def ppo_loss(log_probs, log_probs_old, entropy, ret, val, newval, adv, config):
    logratio = log_probs - log_probs_old.reshape(-1)
    ratio = logratio.exp()
    ...
    return loss, (pg_loss, v_loss, entropy_loss, old_approx_kl, approx_kl, clipfrac)
...
def train(data):
    ...
    for epoch in range(config.update_epochs):
        ...
        for mb in range(experience.num_minibatches):
            with profile.train_misc:
                #TODO: ascertain that enough data was sampled
            ...
            with profile.train_misc:
                #TODO: ascertain that updates are done correctly
                loss, aux = data.optimizer.update_gradient_estimates(
                    ppo_loss, (newlogprob, log_probs, entropy, ret, val, newvalue, adv), config
                )
                pg_loss, v_loss, entropy_loss, old_approx_kl, approx_kl, clipfrac = aux
