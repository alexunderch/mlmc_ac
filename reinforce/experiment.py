...
def run_reinforce_experiment(
    ...
    def reinforce_update(policy_state, env_state, env_params, observations, actions, returns, rng):
        @jax.jit
        def loss_fn(params, observations, actions, returns):
            apply_fn = jax.vmap(policy_state.apply_fn, in_axes=(None, 0))
            log_probs = Categorical(logits=apply_fn({'params': params}, observations)).log_prob(actions)
            return -jnp.sum(log_probs * jax.lax.stop_gradient(returns))

        loss_grad_fn = jax.value_and_grad(loss_fn)
        if mlmc_correction:
            #truncated geometric distribution
            rng, _rng, reset_rng = jax.random.split(rng, 3)
            N = 8
            p = 0.5
            ...
            if 2**J <= batchsize_limit and J > 0:
                initial_obs, initial_env_state = jit_reset(reset_rng, env_params)
                (_, _, env_params, rng, policy_state, ep_len), (observations, actions, rewards, dones, _) = jax.lax.scan(
                    step_fn, (initial_obs, initial_env_state, env_params, _rng, policy_state, jnp.array(0)), xs=None, length=J_t
                )
                returns = _calculate_returns(rewards, dones)

                _, mlmc_grads_t = loss_grad_fn(policy_state.params, observations, actions, returns) #t
                _, mlmc_grads_tm1 = loss_grad_fn(policy_state.params, observations[:J_tm1], actions[:J_tm1], returns[:J_tm1]) #tm1
        ...
        return loss_value, policy_state, observation, env_state
    ...
    for i_episode in range(1, int(n_training_episodes)+1):
        ...
        loss, policy_state, state, env_state = reinforce_update(
            policy_state,
            initial_env_state,
            env_params,
            states,
...
def main(cfg: DictConfig) -> None:
    ...
    run_reinforce_experiment(optimiser=optimisers[dict_config["optimiser"]], **dict_config["experiment"], seed=dict_config["seed"])
